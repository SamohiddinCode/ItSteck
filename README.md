...
## Бесплатный деплой — Render + Neon

База отдельно от приложения не случайно: бесплатный PostgreSQL у Render
удаляется примерно через месяц вместе с данными, а free-тариф Neon живёт
постоянно.

1. **[neon.tech](https://neon.tech)** — создать проект, скопировать строку
   подключения.
2. **[render.com](https://render.com)** → **New → Blueprint** → указать этот
   репозиторий. Render прочитает `render.yaml` и соберёт `Dockerfile.web` —
   один образ, где фронтенд уезжает внутрь бэкенда.
3. В переменных сервиса задать:
   - `DATABASE_URL` — строку от Neon (схему `postgresql://` приложение
     приводит к нужному драйверу само);
   - `FIRST_ADMIN_EMAIL` и `FIRST_ADMIN_PASSWORD` — **до первого запуска**,
     администратор создаётся один раз, пока таблица пользователей пуста;
   - `BOT_TOKEN` и `CHAT_ID`, если нужны уведомления.
   `SECRET_KEY` Render сгенерирует сам.

На бесплатном тарифе сервис засыпает после ~15 минут без запросов, и первый
запрос после сна идёт 30–50 секунд. Тот же образ без изменений заводится на
Koyeb, Fly и любой виртуалке:
